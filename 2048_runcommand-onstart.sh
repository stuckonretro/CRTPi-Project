...
if [[ "$system" == "arcade" ]] || [[ "$system" == "fba" ]] || [[ "$system" == "mame-libretro" ]] || [[ "$system" == "neogeo" ]] ; then
	...
	if ! grep -q "custom_viewport_width" "$rom_fp"".cfg"; then 
		echo -e "custom_viewport_width = ""\"2048\"" >> "$rom_fp"".cfg"  2>&1
	fi
fi
...
#			vcgencmd hdmi_timings 2048 1 180 202 300 240 1 3 5 14 0 0 0 60 0 42954545 1 > /dev/null #Retrotink 2048x240p Timing Original
			...
			fbset -depth 8 && fbset -depth 16 -xres 2048 -yres 240 > /dev/null #Retrotink 24b depth
else
...
#	vcgencmd hdmi_timings 320 1 23 30 34 240 1 4 3 15 1 0 0 60 0 6400000 1 > /dev/null #Retrotink 320x240p Timing Original
	...
	tvservice -c "DMT 87" 
	fbset -depth 8 && fbset -depth 16 -xres 320 -yres 240 > /dev/null #Retrotink 24b depth
fi
