...
// slightly modified by Sakitoshi
...
void main()
{
   ...
   if (InputSize.x > 160.0 && InputSize.y > 144.0)
      vTexCoord = middle + diff * vec2(0.857, 0.6428);
   else
...
void main()
{
   ...
   if (screen_coord.x > 0.0 && screen_coord.x < 1.0 && screen_coord.y > 0.0 && screen_coord.y < 1.0) // needed to prevent the screen from repeating at the edges on raspberry pi
      ...
      screen *= 0.0;
   if (vTexCoord.x > 0.0 && vTexCoord.x < 1.0 && vTexCoord.y > 0.0 && vTexCoord.y < 1.0) // needed to prevent the border from repeating at the edges on raspberry pi
