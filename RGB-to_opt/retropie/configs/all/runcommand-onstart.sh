...
#		vcgencmd hdmi_timings 2048 1 180 202 300 240 1 3 5 14 0 0 0 60 0 42954545 1 > /dev/null; #Retrotink 2048x240p Timing Original
		...
		fbset -depth 24 && fbset -depth 24 -xres 2048 -yres 240 > /dev/null; #Retrotink 24b depth 
	# change timings for 320.txt to 1920x240p
	elif { ! echo "$3" | grep -q -wi "$TwoFiveSix" || echo "$TwoFiveSix" | grep -q empty; } && ! echo "$TwoFiveSix" | grep -q -xi "all" && { echo "$3" | grep -q -wi "$ThreeTwenty" || echo "$ThreeTwenty" | grep -q empty; }; then > /dev/null
#		vcgencmd hdmi_timings 1920 1 152 247 280 240 1 3 7 12 0 0 0 60 0 40860000 1 > /dev/null; #Pi2SCART Standard 1920x240p Timing Original
		...
		tvservice -c "DMT 87" > /dev/null;
		fbset -depth 24 && fbset -depth 24 -xres 1920 -yres 240 > /dev/null; #Retrotink 24b depth
...
#		vcgencmd hdmi_timings 1600 1 85 157 192 240 1 4 3 15 0 0 0 60 0 32000000 1 > /dev/null #VGA666 Standard 1600x240p Timing Original
		...
		tvservice -c "DMT 87" > /dev/null
		fbset -depth 24 && fbset -depth 24 -xres 1600 -yres 240 > /dev/null #Retrotink 24b depth
	fi
...
#	vcgencmd hdmi_timings 320 1 23 30 34 240 1 4 3 15 1 0 0 60 0 6400000 1 > /dev/null #Retrotink 320x240p Timing Original
	...
	tvservice -c "DMT 87" 
	fbset -depth 24 && fbset -depth 24 -xres 320 -yres 240 > /dev/null #Retrotink 24b depth
fi
