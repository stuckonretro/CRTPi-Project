...
# Set rom_resolution_height for 480p and 448p roms
	if [ $rom_resolution_height == "480" ]; then
		rom_resolution_height="480"
	elif [ $rom_resolution_height == "448" ]; then
		rom_resolution_height="448"
	fi	
...
# change timings for 256.txt to 2048x480p
	if { ! echo "$3" | grep -q -wi "$ThreeTwenty" || echo "$ThreeTwenty" | grep -q empty; } && ! echo "$ThreeTwenty" | grep -q -xi "all" && { echo "$3" | grep -q -wi "$TwoFiveSix"; } then > /dev/null
		vcgencmd hdmi_timings 2048 1 180 202 300 480 1 6 10 28 0 0 0 60 0 85909090 1 > /dev/null; #CRTPi 2048x480p Timing Adjusted
		tvservice -c "DMT 87" > /dev/null;
		fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 2048 -yres 480 > /dev/null; #24b depth
		
# change timings for 320.txt to 1920x480p
		...
		vcgencmd hdmi_timings 1920 1 137 247 295 480 1 6 14 24 0 0 0 60 0 81720000 1 > /dev/null #CRTPi 1920x480p Timing Adjusted
		tvservice -c "DMT 87" > /dev/null
		fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 1920 -yres 480 > /dev/null #24b depth
		
# change timings for 256x224 systems to 2048x448p
	elif 
		[[ "$system" == "snes" ]] || 
		[[ "$system" == "nes" ]] || 
		[[ "$system" == "romhacks" ]] ; then
			vcgencmd hdmi_timings 2048 1 180 202 300 448 1 22 10 44 0 0 0 60 0 85909090 1 > /dev/null; #CRTPi 2048x448p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null;
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 2048 -yres 448 > /dev/null; #24b depth
			
# change timings for 256x240 systems to 2048x480p
	elif 
		[[ "$system" == "fds" ]] || 
		[[ "$system" == "pcengine" ]] || 
		[[ "$system" == "pce-cd" ]] ||
		...
		[[ "$system" == "wonderswancolor" ]] ; then
			vcgencmd hdmi_timings 2048 1 180 202 300 480 1 6 10 28 0 0 0 60 0 85909090 1 > /dev/null; #CRTPi 2048x480p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null;
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 2048 -yres 480 > /dev/null; #24b dept
			
# change timings for 256x192 systems to 2048x384p
	elif 		
		[[ "$system" == "mastersystem" ]] || 
		[[ "$system" == "sg-1000" ]] ; then
			vcgencmd hdmi_timings 2048 1 180 202 300 384 1 54 10 76 0 0 0 60 0 85909090 1 > /dev/null #CRTPi 2048x384p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 2048 -yres 384 > /dev/null #24b depth
			
# change timings for 320x224 & 384x224 systems to 1920x448p
	elif 
		[[ "$system" == "megadrive" ]] || 
		[[ "$system" == "segacd" ]] || 
		[[ "$system" == "sega32x" ]] || 
		[[ "$system" == "fba" ]] || 
		[[ "$system" == "neogeo" ]] ; then
			vcgencmd hdmi_timings 1920 1 137 247 295 448 1 22 14 40 0 0 0 60 0 81720000 1 > /dev/null #CRTPi 1920x448p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 1920 -yres 448 > /dev/null #24b depth
			
# change timings for 320x200/640x400 systems to 1920x400p
	elif 
		[[ "$system" == "quake" ]] || 
		[[ "$system" == "doom" ]] ; then
			vcgencmd hdmi_timings 1920 1 137 247 295 400 1 46 14 64 0 0 0 60 0 81720000 1 > /dev/null #CRTPi 1920x400p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 1920 -yres 400 > /dev/null #24b depth
			
# change timings for 320x192 systems to 1920x384p
	elif 
		[[ "$system" == "atari2600" ]] || 
		[[ "$system" == "atari5200" ]] || 
		[[ "$system" == "atari800" ]] || 
		...
		[[ "$system" == "zxspectrum" ]] ; then
			vcgencmd hdmi_timings 1920 1 137 247 295 384 1 54 14 72 0 0 0 60 0 81720000 1 > /dev/null #CRTPi 1920x384p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 1920 -yres 384 > /dev/null #24b depth
			
# change timings for 320x240/640x480 systems to 1920x480p
	elif 
		[[ "$system" == "psx" ]] || 
		[[ "$system" == "dreamcast" ]] || 
		[[ "$system" == "saturn" ]] || 
		...
		[[ "$system" == "cavestory" ]] ; then 
			vcgencmd hdmi_timings 1920 1 137 247 295 480 1 6 14 24 0 0 0 60 0 81720000 11 > /dev/null #CRTPi 1920x480p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 1920 -yres 480 > /dev/null #24b depth
			
# default to 1600x480p for all other lr- systems
	else
		vcgencmd hdmi_timings 1600 1 73 157 204 480 1 8 6 30 0 0 0 60 0 64000000 1 > /dev/null #CRTPi 1600x480p Timing Adjusted
		tvservice -c "DMT 87" > /dev/null
		fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 1600 -yres 480 > /dev/null #24b depth
	fi

# otherwise -- determine and set variable resolutions for non-libretto cores	
elif
# for eDuke32 switch to 640x200p
	[[ "$system" == "eduke32" ]] ||
	[[ "$system" == "duke3d" ]] ||
	[[ "$system" == "scummvm" ]] ||
	[[ "$system" == "dosbox" ]] ||
